...
async function deleteRecentArticles() {
  try {
    ...
    if (refError) {
      ...
      const { data: allTopics, error: allTopicsError } = await supabase.schema("app").from("topics").select("id, name");
      ...
      if (!allTopicsError && allTopics) {
        ...
        if (orphanedTopics.length > 0) {
          ...
          const { error: cleanupError } = await supabase.schema("app").from("topics").delete().in("id", orphanedIds);

...
export class ArticleService {
  ...
  async validateSource(sourceId: string): Promise<boolean> {
    const { data, error } = await this.supabase.from("rss_sources").select("id").eq("id", sourceId).single();
  ...
  async validateTopics(topicIds: string[]): Promise<{ valid: boolean; invalidIds: string[] }> {
    ...
    const { data, error } = await this.supabase.from("topics").select("id").in("id", topicIds);
  ...
  async createArticle(command: CreateArticleCommand): Promise<ArticleEntity> {
    ...
    if (command.topicIds && command.topicIds.length > 0) {
      ...
      const { error: associationError } = await this.supabase.from("article_topics").insert(associations);

      if (associationError) {
        // Rollback: delete the article
        await this.supabase.from("articles").delete().eq("id", article.id);
